...
class Subscription extends IDisposable {
  ...
  Future<SubscriptionModel> create(
      {required SubscriptionRequestMessage request,
  ...
  Future<SubscriptionModel> delete(
      {required String id, String customApiToken = ''}) async {
    var result = await apiResource.delete(id: id, apiUserToken: customApiToken);
    return SubscriptionModel.fromMap(result);
  }

  Future<SubscriptionModel> put(
      {required String id, required SubscriptionModel model}) async {
