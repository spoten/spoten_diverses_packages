...
import '../../domain/interfaces/diposable_interface.dart';
import '../../utils/constantes.dart';
import '../models/request/account_configuration_request_message.dart';
import '../models/request/bank_verification_request_message.dart';
import '../models/request/verify_account_request_message.dart';
import '../models/response/account_request_withdraw_response.dart';
import '../models/response/get_account_response_message.dart';
import '../models/response/simple_response_message.dart';
import '../models/response/verify_account_response_message.dart';
...
class Account extends IDisposable {
  ...
  Future<AccountRequestWithdrawResponse> requestWithdrawAll({
    ...
    var resultAccountBalance = await apiResource.getById(id: targetAccountId);
    var accountBalanceValue =
        GetAccountResponseMessage.fromMap(resultAccountBalance);
    var convertedValue = double.parse(accountBalanceValue.balance
        .replaceAll(CurrencySymbol.BRL, '')
        .replaceAll(",", "."));
    var result = await apiResource.post(
        data: {"amount": convertedValue},
        partOfUrl: "/$targetAccountId/request_withdraw",
        apiUserToken: customUserApiToken);
    return AccountRequestWithdrawResponse.fromMap(result);
  }
