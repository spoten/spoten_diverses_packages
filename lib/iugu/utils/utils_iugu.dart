class UtilsIugu {
  ...
  bool validateCreditCardNumber(String creditCard) {
    ...
    if (creditCard.isEmpty || reg.hasMatch(creditCard)) return false;
  ...
  bool validateExpirationString(String date) {
    var monthYear = getMonthYearByFullExpiration(date);

    return validateExpiration(monthYear[0], monthYear[1]);
  }
  ...
  List<String> getMonthYearByFullExpiration(String date) {
    ...
    return 2 == dateSplit.length ? [date[0], date[1]] : [];
  }
  ...
  bool validateExpiration(String _month, String _year) {
    ...
    } else if (year > today.year && (month > 12 || 1 > month)) {
      return false;
    }
